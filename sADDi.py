...
class Generator(nn.Module):
    def __init__(self, arg: Args, is_train=False):
        ...
        self.upsample1 = UpsampleBlock(260, 128, 4) if not is_train else UpsampleBlock(256, 128, 4)
...
class Discriminator(nn.Module):
    def __init__(self, arg, is_train=False):
        ...
        self.net = nn.Sequential(
            ...
            nn.Linear(100, 5) if not is_train else nn.Linear(100, 1),  # 5 = 4(情绪类别) + 1
        )
...
class sADDiTrainer:
    def __init__(self, arg: Args, src_dataset_name, tgt_dataset_name):
        ...
        self.encoder_final_path = f"models/ADDi/encoder_final.pt"
        self.encoder_train_path = f"models/ADDi/encoder_train.pt"
        self.classifier_path = "models/ADDi/classifier.pt"
    ...
    def pretext(self):
        ...
        discriminator_optimizer = torch.optim.Adam(
            [{"params": discriminator.parameters(), "lr": 6e-4}]
        )
        ...
        for epoch in range(self.pretext_epochs):
            ...
            for real_x, real_label in self.src_train:
                ...
                generator.train()
                discriminator.eval()
                x_no = torch.randn([batch_size, self.feature_dim, self.mini_seq_len]).to(device)  # 不带情绪的数据
                z_p = encoder(x_no)
                ...
                # fake_label = torch.cat(
                #     [y_p, torch.zeros([self.batch_size, 1]).type(torch.FloatTensor).to(device)], dim=1)
                # real_label = torch.cat(
                #     [real_label, torch.ones([self.batch_size, 1]).type(torch.FloatTensor).to(device)],
                #     dim=1)
                label = torch.cat([torch.argmax(real_label, dim=1), torch.full([batch_size], 4).to(device)], dim=0)
                fake_x = generator(torch.cat([z_p, y_p], dim=1).unsqueeze(-1))
                ...
                generator.train()
                discriminator.eval()
                x_no = torch.randn([batch_size, self.feature_dim, self.mini_seq_len]).to(device)
                z_p = encoder(x_no)
                y_p = self.pseudo_label(x_no.shape[0]).to(device)  # 伪标签
