...
class MultiTrainer:
    def __init__(self, args: Args):
        ...
        self.best_path = "../models/semi/MODMA_best.pt"
        self.encoder_final_path = f"models/semi/encoder.pt"
        self.model_path = "../models/semi/MODMA.pt"
        self.pretrain_path = "../models/semi/pretrain.pt"
        self.semi_train_path = "models/semi/semi_train.pt"
        self.pretrain_best_path = "../models/semi/pretrain_best.pt"
    ...
    def pretrain(self):
        loader = self.get_loader(dataset_name[1], batch_size=self.batch_size, split_rate=[0.6, 0.2, 0.2])
        arg = Args()
        ...
        train_num = len(loader[0].dataset)
        val_num = len(loader[1].dataset)
        best_val_accuracy = 0
        metric = Metric()
    ...
    def semi_train(self):   # 大概95.5%（测试集）
        loader = self.get_loader(dataset_name[0], batch_size=self.batch_size, split_rate=[0.2, 0.1, 0.7])
        arg = Args()
        arg.num_class = num_class
        model = MModel(arg, index=0)
        model = model.to(device)
        optimizer = self.get_optimizer(arg, model.parameters(), 4e-4)
        scheduler = self.get_scheduler(optimizer, arg)
        train_num = len(loader[0].dataset)
        val_num = len(loader[1].dataset)
        best_val_accuracy = 0
    ...
    def train(self):
        loader = [self.get_loader(dataset_name[0], batch_size=self.batch_size, split_rate=[0.2, 0.1, 0.7]),
                  self.get_loader(dataset_name[1], batch_size=self.batch_size, split_rate=[0.6, 0.2, 0.2])]
        arg.step_size = 30
        arg.gamma = 0.3
        ...
        # model.prepare.load_state_dict(src_model.prepare.state_dict())
        # model.shareNet.load_state_dict(src_model.shareNet.state_dict())
        src_model.eval()
        # parameter = [
        #     {'params': model.specialNet.parameters(), 'lr': arg.lr},
        # ]
        spec_optimizer = self.get_optimizer(arg, model.parameters(), lr=arg.lr)
        scheduler = self.get_scheduler(spec_optimizer, arg)
        ...
        for epoch in range(self.epochs):
            ...
            for batch in loader[0][0]:
                loss, correct_num = self.train_step(model, spec_optimizer, batch)
                train_acc += correct_num.cpu().numpy()
                train_loss += loss.data.item()
            ...
            train_acc = train_acc / len(loader[0][0].dataset)
            ...
            with torch.no_grad():
                for batch in loader[0][1]:
                    ...
                    val_acc += correct_num.cpu().numpy()
                    val_loss += loss.data.item()
            val_acc = val_acc / len(loader[0][1].dataset)
            val_loss = val_loss / math.ceil(int(len(loader[0][1].dataset)) / self.batch_size)
            metric.val_acc.append(val_acc)
            metric.val_loss.append(val_loss)
            ...
            train_acc = 0
            train_loss = 0
        np.save("../results/data/semi/MODMA.npy", metric.item())
        torch.save(model, self.model_path)

    def test(self, path=None):
        if path is None:
            path = get_newest_file("../models/semi/")
            print(f"path is None, choose the newest model: {path}")
        if not os.path.exists(path):
            print(f"error! cannot find the {path}")
            return
        loader = self.get_loader(dataset_name[0], batch_size=self.batch_size, split_rate=[0.2, 0.2, 0.6])
        ...
        metric.test_acc.append(test_acc)
        metric.test_loss.append(test_loss)
        np.save("../results/data/semi/test.npy", metric.item())
