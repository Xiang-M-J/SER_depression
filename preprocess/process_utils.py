...
def process_DAIC(path: str, code, frame_length=0.05, duration=10, resample_rate=16000, threshold=1,
                 order=3, overlap=2.5):
    """
    处理DAIC数据库
    Args:
...
def MODMA_code(class_name):
    ...
    conf = np.load(path[:-12] + r"preprocess\data\conf.npy", allow_pickle=True).item()
...
if __name__ == "__main__":
    ...
    # process_MODMA(path="MODMA", duration=10, new_wavs_path="MODMA_Plus")
    # process_DAIC("DAIC", code=DAIC_code, duration=6, overlap=0)
    data_distribution(path="datasets/CASIA", step=1)
    load_MultiDataset_V1(path="datasets/CASIA", code=CASIA_code)
    pass
