...
class myWavLoader(dataset.Dataset):
    ...
    def __init__(self, files, labels, name, duration=10) -> None:
...
class PretrainDataModule:
    def __init__(self, name, num_class, label_path, data_dir, spilt_rate=None, duration: int = 10):
        super().__init__()
    ...
    def setup(self):
        ...
        self.train_dataset = myWavLoader(train_wavs, train_labels, self.name, self.duration)
        self.val_dataset = myWavLoader(val_wavs, val_labels, self.name, self.duration)
        self.test_dataset = myWavLoader(test_wavs, test_labels, self.name, self.duration)
