...
class MultiTrainer:
    ...
    def get_mmd_loss(model, src_model, train_batch):
        ...
        x_f = src_model[0](train_batch[1][0])
        x_b = src_model[0](torch.flip(train_batch[1][0], dims=[-1]))
        _, x, _ = src_model[1](x_f, x_b)
    ...
    def train(self):
        ...
        pretrain_model_path = "CASIA.pt"
        tmp = torch.load(pretrain_model_path)
        src_model = torch.nn.Sequential(
            tmp.prepare,
            tmp.generalExtractor
        ).to(device)
        src_model.train()
